...
from myapp.app.models.pokeOptions import PokeOptions

from weboob.browser.pages import HTMLPage
from weboob.capabilities.base import BaseObject, Field, StringField, IntField
from weboob.browser.elements import TableElement, ItemElement, method
from weboob.browser.filters.standard import CleanText, CleanDecimal, TableCell, Field, Type
from weboob.browser.filters.html import Attr

class PokemonPage(HTMLPage):
    @method
    class get_options(ItemElement): 
        klass = PokeOptions

        def obj_abilityOptions(self):
           list_options = self.page.doc.xpath('//*[@id="ability"]/option')
           dict_options = dict()
           return {item.attrib["value"]: CleanText(item)(self) for item in list_options}

        def obj_tierOptions(self):
            list_options = self.page.doc.xpath('//*[@id="tier"]/option')
            dict_options = dict()
            return {item.attrib["value"]: CleanText(item)(self) for item in list_options}
